...
void Elas::process (uint8_t* h_img0,uint8_t* h_img1,float* h_disp0,float* h_disp1,const int32_t* dims)
{
    ...
    // 拷贝图像数据到GPU显存中
#if PROFILE
    timer.start("Upload Images To GPU");
#endif
	// uint8_t指代unsigned char
...
void Elas::computeDescriptor(Dev2DVector<uint8_t> &d_img,Dev2DVector<uint8_t> &d_desc)
{
    // 为Sobel滤波器分配水平和竖直方向的显存
